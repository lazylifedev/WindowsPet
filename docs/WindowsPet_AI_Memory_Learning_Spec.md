...
- Product integration now wires `ProactiveRuntime` into the real Qt runtime for startup and low-frequency idle/lunch candidate checks, exposes local off/low/normal settings, and sends only `RelationshipService.bounded_context()` to the chat provider. Speech bodies remain non-persistent and all anti-annoyance gates remain in force.
- File Rename MVP and read-only UI Automation foundation are implemented as separate local domains. File Rename binds canonical identity and before/after preview through the existing confirmation/grant flow; UIA only returns bounded redacted structure and never clicks or types.
...
- **0.9 — 2026-08-09:** Recorded production Proactive/Personality runtime wiring, bounded relationship context injection, local File Rename identity/verification flow, fixed structured operation catalog, and read-only UI Automation foundation. Cloud transport, real user-file mutation, administrative changes, and UIA side effects remain excluded.
