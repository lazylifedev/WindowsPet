...
class ToolDispatcher:
    ...
    def parse_windows_inspection(arguments: str | dict) -> WindowsInspectionRequest:
        ...
        if (query is not None and (not isinstance(query, str) or len(query) > 100 or any(ch in query for ch in ("\0", "\r", "\n", "\t")) or any(unicodedata.category(ch) == "Cc" for ch in query))) or type(maximum) is not int or not 1 <= maximum <= 100 or (area is WindowsInspectionArea.NETWORK and query is not None) or (area is WindowsInspectionArea.REGISTRY and query not in (None, "app_paths", "installed_apps")) or (area is WindowsInspectionArea.WINGET and (query is None or not query.strip())): raise ValueError("inspect_windows の引数が不正です。")
