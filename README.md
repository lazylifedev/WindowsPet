...
- チャットからの読み取り専用Windows調査（プロセス、サービス、ネットワーク）
...
チャットで「動いているプロセスを確認して」「Windows Update のサービス状態を確認して」「ネットワーク設定を確認して」のように依頼すると、AI は必要に応じて `inspect_windows` を使います。PowerShell による調査は読み取り専用です。プロセスの終了、サービスの開始・停止・設定変更、ネットワーク設定変更、アプリ起動、資格情報や保存済み Wi-Fi プロファイルの取得は行いません。調査の開始・完了はUIに表示され、監査ログには結果本体や機密値を含めません。
